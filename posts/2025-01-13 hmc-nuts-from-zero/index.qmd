...
ggplot(df, aes(x, -y, z = z)) +
  geom_contour_filled(bins = 20, na.rm = TRUE, show.legend = FALSE) +
  scale_y_continuous(limits = c(-6,1), expand = c(0,0), 
                     oob = scales::squish_infinite) +
  scale_x_continuous(limits = c(-3,3), expand = c(0,0), 
                     oob = scales::squish_infinite) +
  labs(title = "Banana-Shaped Distribution") +
...
ggplot(plot_df, aes(iter, value, col = variable)) +
  ...
  annotate(geom = "text", x = 1950, y = -3.75, label = "\"Burn-in\" Period", hjust = 1,
           size = 3.5) +
