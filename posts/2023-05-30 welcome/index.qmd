...
[This symbol "$\sim$" means "distribuited as"]{.aside}
...
To calculate the posterior probability, we need to normalize the product of the likelihood and prior, which involves integrating over all possible values of H.[^pdf] However, in this case, we can use a shortcut because the prior distribution is conjugate to the binomial distribution,[^conjugacy] so the posterior distribution will also follow a Beta distribution:

[^pdf]: This is given that the product of both the prior and the likelihood keep the same shape as the posterior, with the exception that it doesn't add up to 1, so is not a probability density function. For this to happen it needs to be normalized using integration.

[^conjugacy]: When using a Beta distribution as our prior belief and observing new data from a coin toss, we find that the posterior distribution also follows a Beta distribution. This convenient property simplifies calculations. It's like a mathematical shortcut that saves time and effort.
